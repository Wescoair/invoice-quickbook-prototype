...
public class CamelCaseToSnakeCaseNamingStrategy extends PhysicalNamingStrategyStandardImpl {
	...
	public static final String CAMEL_CASE_REGEX = "([a-z]+)(([A-Z]|[0-9])+)";
	...
	private Identifier formatIdentifier(Identifier identifier) {
		if (identifier != null) {
			...
			if ("accountNumber2".equals(name)) {
				formattedName = "account_number2";
			} else {
				formattedName = name.replaceAll(CAMEL_CASE_REGEX, SNAKE_CASE_PATTERN).toLowerCase();
			}
